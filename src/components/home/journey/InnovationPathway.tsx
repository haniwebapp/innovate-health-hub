...
export function InnovationPathway({ inView }: PathwayProps) {
  return (
    <div className="absolute hidden lg:block">
      ...
      {/* Moving particle along the path - active only when in view */}
      {inView && (
        <motion.div
      ...
      {/* Additional moving particle with different timing */}
      {inView && (
        <motion.div
          className="absolute top-20 w-2 h-2 rounded-full bg-moh-gold shadow-sm shadow-moh-gold/30"
          style={{ 
            marginTop: "-4px", 
            marginLeft: "-4px" 
          }}
