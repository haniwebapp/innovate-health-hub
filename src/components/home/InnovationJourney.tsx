...
export default function InnovationJourney() {
  ...
  return <section className="py-20 bg-moh-lightGreen/30 relative" ref={ref}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 my-0 py-0">
        ...
        <div className="max-w-4xl mx-auto relative pb-10">
          ...
          <motion.div className="absolute left-1/2 top-0 bottom-0 w-0.5 bg-moh-lightGreen" style={{
          marginLeft: "-1px"
        }} initial={{
          scaleY: 0,
          transformOrigin: "top"
          ...
          <div className="space-y-20">
            {timelineItems.map((item, index) => <div key={index} className="relative">
                <TimelineItem isLeft={index % 2 === 0} index={index} isActive={item.isActive} importance={item.importance} duration={item.duration}>
                  <div className={`${index % 2 === 0 ? 'text-right' : 'text-left'}`}>
                    ...
                    {item.completionPercent > 0 && <motion.div className="mt-2" initial={{
                  opacity: 0
                }} animate={inView ? {
                  opacity: 1
                } : {
                  ...
                  delay: 1 + index * 0.1
                        ...
                        <div className="flex items-center gap-2">
                          <div className="grow h-1 bg-gray-200 rounded-full overflow-hidden">
                            <motion.div className={`h-full ${item.completionPercent > 80 ? 'bg-moh-green' : item.completionPercent > 40 ? 'bg-moh-gold' : 'bg-moh-lightGold'}`} initial={{
                        width: 0
                      }} animate={inView ? {
          ...
          {/* End dot */}
          <motion.div className="absolute left-1/2 bottom-0 w-5 h-5 rounded-full bg-moh-green -ml-2.5" initial={{
          scale: 0
        }} animate={inView ? {
          scale: 1
        } : {}} transition={{
          ...
          delay: 1.5
            ...
            <motion.div className="absolute inset-0 w-full h-full rounded-full bg-moh-green opacity-50" animate={{
            scale: [1, 1.8, 1],
            opacity: [0.5, 0, 0.5]
          }} transition={{
            duration: 2,
