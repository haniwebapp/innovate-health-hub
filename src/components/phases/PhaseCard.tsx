...
export function PhaseCard({ 
  ...
  // Determine background color classes based on props
  const getActiveColorClass = () => {
    if (phase.isActive) return activeColor;
    if (phase.completionPercent === 100) return completedColor;
    return accentColor;
  };

  const getCompletionColorClass = () => {
    return phase.completionPercent === 100 ? completedColor : activeColor;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div 
          className={`absolute bottom-0 left-0 h-1 transition-all duration-1000 ease-out ${getCompletionColorClass()}`}
          style={{ 
        ...
        {phase.isActive && (
          <div className="absolute top-3 right-3">
            <span className="flex h-3 w-3">
              <span className={`animate-ping absolute h-full w-full rounded-full opacity-75 ${activeColor}`}></span>
              <span className={`relative rounded-full h-3 w-3 ${activeColor}`}></span>
            </span>
          </div>
        )}
            ...
            <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
              phase.isActive 
                ...
                : phase.completionPercent === 100 
                  ? completedColor + "/20 text-moh-gold" 
                  : accentColor + "/20 text-moh-darkGreen"
            }`}>
              {phase.completionPercent === 100 ? (
