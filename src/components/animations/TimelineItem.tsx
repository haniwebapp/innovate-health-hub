...
export function TimelineItem({ 
  ...
  const getNodeColor = () => {
    ...
    switch (importance) {
      case 'high': return "bg-moh-darkGreen";
      case 'low': return "bg-moh-lightGreen";
      default: return "bg-moh-green";
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <motion.div 
          className={`${getNodeSize()} rounded-full ${getNodeColor()} z-10`}
          initial={{ scale: 0 }}
        ...
        {isActive && (
          <motion.div 
            className="absolute top-0 left-0 w-full h-full rounded-full bg-moh-gold/50 z-0"
            animate={{
              scale: [1, 1.8, 1],
              opacity: [0.6, 0, 0.6]
            }}
        ...
        {duration && (
          <motion.div 
            className={`absolute h-${Math.min(40, Math.max(4, duration))} w-1 ${
              isActive ? "bg-moh-gold/40" : "bg-moh-lightGreen/50"
            } rounded-full z-0`}
            style={{ 
              top: '100%',
              left: '50%',
      ...
      <motion.div 
        className={`absolute top-1/2 left-0 right-0 h-0.5 ${
          isActive ? "bg-moh-gold" : "bg-moh-lightGreen"
        } z-0`}
        initial={{ scaleX: 0 }}
