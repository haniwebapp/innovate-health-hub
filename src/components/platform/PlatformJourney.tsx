...
import { ArrowRight, Check, Clock, ExternalLink, Info, Layers } from "lucide-react";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
    ...
    <div className="container mx-auto px-4 py-8">
      <header className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          <Layers size={24} className="text-moh-green" />
          <h1 className="text-3xl font-bold">Platform Capabilities Roadmap</h1>
        </div>
        <p className="text-muted-foreground">
          Explore our platform's AI-powered features and their implementation timeline
        </p>
      </header>
      ...
      <Tabs defaultValue="phases" value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="phases" className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {phases.map((phase) => {
              const platformPhaseCaps = phase.capabilities.filter(cap => cap.isPlatformFeature);
              return (
                <Card key={phase.id} className={`border-l-4 ${phase.status === 'in-progress' ? 'border-l-moh-green' : 'border-l-slate-300'}`}>
                  <CardHeader>
                    <div className="flex justify-between items-center">
                      <CardTitle>{phase.name}</CardTitle>
                      <Badge variant={phase.status === 'in-progress' ? 'default' : 'outline'} className={phase.status === 'in-progress' ? 'bg-moh-green' : ''}>
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-muted-foreground">{platformPhaseCaps.length}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <span className="font-medium">Implemented</span>
                        <span className="text-muted-foreground">
                          {platformPhaseCaps.filter(c => c.status === AICapabilityStatus.LIVE).length}
                        </span>
                      </div>
                    </div>
                  </CardContent>
          ...
          <Card className="bg-slate-50">
            <CardHeader>
              <CardTitle className="text-lg">Platform Implementation Journey</CardTitle>
              <CardDescription>Our roadmap for implementing platform AI capabilities</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="relative">
                ...
                <div className="space-y-8">
                  <div className="relative pl-12">
                    <div className="absolute left-0 w-8 h-8 bg-moh-green rounded-full flex items-center justify-center text-white">1</div>
                    <h3 className="font-bold text-lg mb-1">Phase 1: Core User Experience</h3>
                    <p className="text-muted-foreground mb-2">Q4 2023 - Q2 2024</p>
                    <div className="space-y-1 text-sm">
                      <p>• Basic AI-powered recommendation features</p>
                      <p>• Knowledge and research assistance</p>
                  ...
                  <div className="relative pl-12">
                    ...
                    <h3 className="font-bold text-lg mb-1">Phase 2: Enhanced Personalization</h3>
                    <p className="text-muted-foreground mb-2">Q2 2024 - Q4 2024</p>
                    <div className="space-y-1 text-sm">
                      <p>• Advanced content personalization</p>
                      <p>• AI-powered forms and submission tools</p>
                      <p>• Personalized learning and recommendations</p>
                    </div>
                  </div>
                  
                  <div className="relative pl-12">
                    <div className="absolute left-0 w-8 h-8 bg-slate-300 rounded-full flex items-center justify-center text-slate-600">3</div>
                    <h3 className="font-bold text-lg mb-1">Phase 3: Advanced User Tools</h3>
                    <p className="text-muted-foreground mb-2">Q4 2024 - Q2 2025</p>
                    <div className="space-y-1 text-sm">
                      <p>• Smart interfaces and voice commands</p>
                      <p>• Advanced decision support tools</p>
                      <p>• Clinical and scientific simulations</p>
                      <p>• Cross-domain mapping and analysis</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <TabsContent value="categories" className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {categories
              .filter(category => platformCapabilities.some(cap => cap.category === category.id))
              .map((category) => {
                const categoryCaps = platformCapabilities.filter(cap => cap.category === category.id);
                  ...
                  <Card key={category.id} className="overflow-hidden">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-lg flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full bg-moh-green/20 flex items-center justify-center text-moh-green">
                          <Layers size={18} />
                        </div>
                        {category.name}
                      </CardTitle>
        ...
        <TabsContent value="list">
          <Card>
            <CardHeader>
              <CardTitle>Platform AI Capabilities</CardTitle>
              <CardDescription>Complete list of platform user-facing AI capabilities</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    .map((category) => (
                      <div key={category.id} className="pb-4 border-b border-slate-200">
                        <h3 className="font-bold text-lg mb-3 flex items-center gap-2">
                          <Layers size={20} className="text-moh-green" />
                          {category.name}
                        </h3>
