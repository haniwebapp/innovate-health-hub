    ...
    <div className="container mx-auto px-4 py-8">
      <header className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-3xl font-bold">AI Capabilities Roadmap</h1>
        </div>
        <p className="text-muted-foreground">
          Explore our AI-powered features and their implementation timeline
        </p>
      </header>
      ...
      <Tabs defaultValue="phases" value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="phases" className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {phases.map((phase) => (
              <Card key={phase.id} className={`border-l-4 ${phase.status === 'in-progress' ? 'border-l-purple-600' : 'border-l-slate-300'}`}>
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <CardTitle>{phase.name}</CardTitle>
                    <Badge variant={phase.status === 'in-progress' ? 'default' : 'outline'} className={phase.status === 'in-progress' ? 'bg-purple-600' : ''}>
                ...
                <CardContent>
                  ...
                  <div className="mt-4 space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{phase.capabilities.length}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="font-medium">Implemented</span>
                      <span className="text-muted-foreground">
                        {phase.capabilities.filter(c => c.status === AICapabilityStatus.LIVE).length}
                      </span>
                    </div>
                  </div>
                </CardContent>
          ...
          <Card className="bg-slate-50">
            <CardHeader>
              <CardTitle className="text-lg">AI Implementation Journey</CardTitle>
              <CardDescription>Our roadmap for implementing 110+ AI capabilities</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="relative">
                ...
                <div className="space-y-8">
                  <div className="relative pl-12">
                    <div className="absolute left-0 w-8 h-8 bg-purple-600 rounded-full flex items-center justify-center text-white">1</div>
                    <h3 className="font-bold text-lg mb-1">Phase 1: Core Capabilities</h3>
                    <p className="text-muted-foreground mb-2">Q4 2023 - Q2 2024</p>
                    <div className="space-y-1 text-sm">
                      <p>• Implementation of foundational AI services</p>
                      <p>• Basic recommendation and analysis features</p>
                      <p>• Integration with core platform functionality</p>
                    </div>
                  </div>
                  
                  <div className="relative pl-12">
                    <div className="absolute left-0 w-8 h-8 bg-slate-400 rounded-full flex items-center justify-center text-white">2</div>
                    <h3 className="font-bold text-lg mb-1">Phase 2: Advanced Features</h3>
                    <p className="text-muted-foreground mb-2">Q2 2024 - Q4 2024</p>
                    <div className="space-y-1 text-sm">
                      <p>• Enhanced prediction and analytics capabilities</p>
                      <p>• Personalization and advanced matchmaking</p>
                      <p>• Advanced NLP for knowledge and content</p>
                    </div>
                  </div>
                  
                  <div className="relative pl-12">
                    <div className="absolute left-0 w-8 h-8 bg-slate-300 rounded-full flex items-center justify-center text-slate-600">3</div>
                    <h3 className="font-bold text-lg mb-1">Phase 3: Specialized Capabilities</h3>
                    <p className="text-muted-foreground mb-2">Q4 2024 - Q2 2025</p>
                    <div className="space-y-1 text-sm">
                      <p>• Complex simulation and forecasting</p>
                      <p>• Cross-domain mapping and analysis</p>
                      <p>• Advanced clinical and scientific capabilities</p>
                      <p>• Strategic policy and national planning features</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <CardContent>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm font-medium">
                      {category.implementedCapabilities} / {category.totalCapabilities}
                    </span>
                  </div>
                  <Progress value={(category.implementedCapabilities / category.totalCapabilities) * 100} className="h-2" />
                </CardContent>
        ...
        <TabsContent value="list">
          <Card>
            <CardHeader>
              <CardTitle>All AI Capabilities</CardTitle>
              <CardDescription>Complete list of 110+ AI capabilities</CardDescription>
            </CardHeader>
            <CardContent>
