...
export default function DashboardSidebar({
  ...
  return <aside className={cn("fixed h-screen z-30 bg-sidebar-background border-r border-sidebar-border transition-all duration-300 ease-in-out", isCollapsed ? "w-16" : "w-64")}>
      <div className="h-full flex flex-col">
          ...
          <div className="p-2">
            <TooltipProvider delayDuration={300}>
              <nav className="grid gap-1">
                {filteredNavItems.map((item, index) => <Tooltip key={index}>
                    <TooltipTrigger asChild>
                      <Link to={item.href} className={cn("flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors", pathname === item.href ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground")}>
                ...
                {user?.email?.endsWith('@moh.gov.sa') && <Tooltip>
                    <TooltipTrigger asChild>
                      <Link to="/dashboard/create-challenge" className={cn("flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors mt-4 bg-moh-lightGreen text-moh-green", pathname === "/dashboard/create-challenge" ? "bg-moh-green text-white" : "")}>
        ...
        <div className="p-2 mt-auto border-t border-sidebar-border">
          <TooltipProvider delayDuration={300}>
            <nav className="grid gap-1">
              {onToggleCollapse && <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="sm" onClick={onToggleCollapse} className="justify-start text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="sm" onClick={handleSignOut} disabled={isLoading} className="justify-start text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
                    {isLoading ? <>
                ...
                {isCollapsed && !isLoading && <TooltipContent side="right">
                    <p>Sign Out</p>
                  </TooltipContent>}
              </Tooltip>
            </nav>
