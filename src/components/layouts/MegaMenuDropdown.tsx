    ...
    >
      <div className="flex p-6 gap-6">
        {categories.map((category, idx) => (
          <div key={idx} className="flex-1 min-w-[250px]">
            <h4 className="font-medium text-[13px] text-moh-darkGreen mb-4 pb-2 border-b border-gray-100">
              {category.title}
            </h4>
            <ul className="space-y-2">
              {category.items.map((item, itemIdx) => (
                <li key={itemIdx}>
                  <Link 
                    to={item.path}
                    className="text-[13px] text-gray-600 hover:text-moh-green flex items-center py-1 transition-colors"
                  >
                    {item.label}
                  </Link>
                </li>
      ...
      <div className="bg-gray-50 p-4 flex justify-between items-center">
        ...
        <Link 
          ...
          className="text-[13px] text-moh-green hover:text-moh-darkGreen font-medium"
        >
          View All
        </Link>
      </div>
