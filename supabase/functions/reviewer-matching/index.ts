...
serve(async (req) => {
  ...
  try {
    ...
    // In a real implementation, we would fetch potential reviewers from the database
    // For this demo, we'll generate mock reviewers based on the required expertise
    ...
    const reviewerMatches = JSON.parse(data.choices[0].message.content);
